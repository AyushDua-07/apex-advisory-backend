...
// Each appointment gets its own room identified by appointmentId.
// Only the two participants (client + consultant) join that room.

const sessionRooms = new Map(); // appointmentId -> Set of socket ids
...
io.on('connection', (socket) => {
  ...
  socket.on('join_session', ({ appointmentId, userId, userName }) => {
    ...
    if (!sessionRooms.has(appointmentId)) {
      sessionRooms.set(appointmentId, new Set());
    }
  ...
  socket.on('disconnect', () => {
    ...
    if (appointmentId) {
      ...
      if (room) {
        ...
        if (room.size === 0) sessionRooms.delete(appointmentId);
      }
